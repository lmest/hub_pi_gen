...
from smccedfw.update_ini import UpdateRadio, UpdateIPConfig
...
def escrever_pedido(requisicao):
    try:
        ...
        if req_json["__instance_type__"][1] == "SensorPanIdConfig":
            ...
            dashboard.PubLog().send_amqp_request_received_from_server(req_json, req_usage_status)
            dashboard.PubLog().send_amqp_request_list(lista_pedidos)
            
        elif req_json["__instance_type__"][1] == "Radio":
            channel = req_json["attributes"]["channel"]
            panid = req_json["attributes"]["panid"]
            ConfigIni().save_new_channel_pan_id(channel, panid)
            os_mng.restart_radio_script()
            
        elif req_json["__instance_type__"][1] == "IpConfig":
            config_server_name = req_json["attributes"]["config_server_name"]
            backend_server_name = req_json["attributes"]["backend_server_name"]
            backend_server_ip = req_json["attributes"]["backend_server_ip"]
            api_token_name = req_json["attributes"]["api_token_name"]
            hub_id = req_json["attributes"]["hub_id"]
            ConfigIni().save_new_ipconfig(config_server_name, backend_server_name, backend_server_ip, api_token_name, hub_id)
            os_mng.restart_radio_script()
