...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/home/pi/log/watchdog.log"),
...
class ProcessWatcher:
    def __init__(self, max_restart, max_log_size_mb):
        ...
        self.process_var = {
                ...
                'Radio': {
                    'process_command' : ["/usr/bin/screen -L -Logfile /home/pi/log/screen_radio.log -dmS radio sudo /home/pi/radio/fwl_hub; true"], 
                    'restart_num' : 0, 
                    'process_status' : STATUS_INIT, 
                    'status' : True,
                    },
    ...
    def check_log_size(self, file_name):
        try:
            if os.path.getsize("/home/pi/log/" + self.logs_name[file_name]) / (1024 * 1024) > self.max_log_size_mb:
                logging.warning("| " + file_name + " Log Limit Size Reached!")
                logging.warning("| Cleaning File\n|")
                open("/home/pi/log/" + self.logs_name[file_name], "w").close()
    ...
    @staticmethod
    def set_led_error():
        subprocess.Popen(["sudo /home/pi/scripts/led_reset.sh; true"], shell=True)
        time.sleep(5)
