...
class Server(object):  
    ...
    def read_global_beacon_fwl(self):
        ...
        try:
            (self.cmd, *pid, vib_x_rms, vib_x_max, vib_z_rms, vib_z_max, temp, bat, self.sensor_zid, num_beacons, ed) = struct.unpack('<B12BLHLHHHBBB', self.msg_received)
            ...
            dashboard.PubLog().send_rssi_zigbee(sensors_id_dict.get_pid_str(self.sensor_zid), ed)
    ...
    def read_data_seg_fwl(self):
        if len(self.msg_received) == self.const['data_seg_msg_size']:
            (ed) = self.msg_received[2]
            self.msg_received = self.msg_received[:2] + self.msg_received[3:]
